...
uint32_t jf_ParseHttpRequest(HttpRequest* httpRequest, const char* buffer, size_t length) {
    ...
    if(!(strcmp(httpRequest->method, "GET") == 0 || strcmp(httpRequest->method, "POST") == 0 || strcmp(httpRequest->method, "PUT") == 0 || strcmp(httpRequest->method, "DELETE") == 0))
        return 0;
    ...
    while(ptr < end){
        ...
        if(httpRequest->headerCount < 32){
            sscanf(header, "%63[^:]: %255[^\n]", httpRequest->headers[httpRequest->headerCount].name, httpRequest->headers[httpRequest->headerCount].value);
